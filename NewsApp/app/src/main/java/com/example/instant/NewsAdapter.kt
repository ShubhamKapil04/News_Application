...
class NewsAdapter(val context: Context, val articles: List<Article>): RecyclerView.Adapter<NewsAdapter.ArticleViewHolder>(){
    ...
    class ArticleViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
        val neweImage = itemView.findViewById<ImageView>(R.id.newsImage)
        val newsTitle = itemView.findViewById<TextView>(R.id.headLines)
    ...
    override fun onBindViewHolder(holder: ArticleViewHolder, position: Int) {
        ...
        Glide.with(context).load(article.urlToImage).into(holder.neweImage)
